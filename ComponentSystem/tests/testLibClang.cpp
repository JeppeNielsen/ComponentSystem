...
void show_all_tokens(const CXTranslationUnit &tu, const CXToken *tokens, unsigned numTokens, const CXCursor* cursors) {
  printf("=== show tokens ===\n");
  printf("NumTokens: %d\n", numTokens);
  for (auto i = 0U; i < numTokens; i++) {
    ...
    printf(" Kind: %s\n", _getTokenKindSpelling(kind));
    CXType type = clang_getCursorType(cursors[i]);
    printf(" cursorType: %i\n", type.kind);
...
int main() {
  ...
  // show tokens
  show_all_tokens(tu, tokens, numTokens, cursors);
